...
if [ $jobid -eq 1 ] ; then
  bat="001"
  job="001/000001"
  opt="-h 1-9145 -r 1-9145 --hashdatalen 80009098"
fi

if [ $jobid -eq 2 ] ; then
  bat="001"
  job="001/000002"
  opt="-h 9146-9424 -r 1-9424 --hashdatalen 2697843"
fi
...
if [ ! -e ../0-mercounts/ecoli-75.ms22.dump ] ; then
  mkdir -p ../0-mercounts
  cd ../0-mercounts
  cd -
fi
...
$bin/overlapInCore \
  ...
  -t 8 \
  -k 22 \
  -k ../0-mercounts/ecoli-75.ms22.dump \
  --hashbits 22 \
  --hashload 0.8 \
  --maxerate  0.045 \
  --minlength 500 \
