...
if [ -e ecoli-75.ctgStore/seqDB.v001.tig ]; then
  exists=true
else
  exists=false
fi
...
if [ -e ./ecoli-75.ctgStore ] ; then
  mv ./ecoli-75.ctgStore ../ecoli-75.ctgStore
fi
...
#
if [ -e ../ecoli-75.ctgStore -a \
   ! -e ../ecoli-75.ctgStore/seqDB.v001.sizes.txt ] ; then
  $bin/tgStoreDump \
    -S ../../ecoli-75.seqStore \
    -T ../ecoli-75.ctgStore 1 \
    -sizes -s 4800000 \
   > ../ecoli-75.ctgStore/seqDB.v001.sizes.txt
fi

if [ -e ../ecoli-75.ctgStore ] ; then
  cd ../ecoli-75.ctgStore
  cd -
fi
