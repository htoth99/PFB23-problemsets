...
if [ -e ./ecoli-75.$jobid.meryl/merylIndex ] ; then
...
if [ -e ecoli-75.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/Users/pfb11/problemsets/genome-assembly/canu-2.2/bin/meryl k=22 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../ecoli-75.seqStore \
    output ./ecoli-75.$jobid.meryl.WORKING \
&& \
mv -f ./ecoli-75.$jobid.meryl.WORKING ./ecoli-75.$jobid.meryl
