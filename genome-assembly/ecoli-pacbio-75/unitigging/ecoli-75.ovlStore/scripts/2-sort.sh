...
#  This script should be executed from unitigging/ecoli-75.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./ecoli-75.ovlStore.BUILDING/$jobname.info -a ! -e ./ecoli-75.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './ecoli-75.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./ecoli-75.ovlStore.BUILDING \
  -S ../ecoli-75.seqStore \
  -C  ./ecoli-75.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
